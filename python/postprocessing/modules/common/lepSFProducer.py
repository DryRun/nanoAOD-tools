...
class lepSFProducer(Module):
    def __init__(self, muonSelectionTag, electronSelectionTag):
        ...
        mu_f = ["%s/src/PhysicsTools/NanoAODTools/data/leptonSF/" % os.environ['CMSSW_BASE'] + f for f in mu_f]
        el_f = ["%s/src/PhysicsTools/NanoAODTools/data/leptonSF/" % os.environ['CMSSW_BASE'] + f for f in el_f]
