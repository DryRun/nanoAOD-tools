...
class BranchSelection:
    def __init__(self,filename):
        ...
        for line in open(filename,'r'):
            ...
            try:
                (op,sel) = line.split()
                if   op == "keep": self._ops.append( (sel, 1) )
                elif op == "drop": self._ops.append( (sel, 0) )
                elif op == "keepmatch": self._ops.append( (re.compile("(:?%s)$" % sel), 1) )
                elif op == "dropmatch": self._ops.append( (re.compile("(:?%s)$" % sel), 0) )
                else:
